...
{
	public class ParasiticNanitesSolutionProj : ModProjectile
	{
		public override void SetStaticDefaults() {
			DisplayName.SetDefault("ParasiticNanite Spray");
		}
		...
		public override void AI() {
			...
			int dustType = ModContent.DustType<Dusts.ParasiticNanitesDust>();
				...
				for (int l = j - size; l <= j + size; l++) {
					if (WorldGen.InWorld(k, l, 1) && Math.Abs(k - i) + Math.Abs(l - j) < Math.Sqrt(size * size + size * size)) {
						...
						if (TileID.Sets.Conversion.Stone[type]) {
							Main.tile[k, l].type = (ushort)ModContent.TileType<ParasiticNanitesStone>();
							WorldGen.SquareTileFrame(k, l, true);
							NetMessage.SendTileSquare(-1, k, l, 1);
						}
						//If the tile is sand, convert to ExampleSand
						else if (TileID.Sets.Conversion.Sand[type]) {
							Main.tile[k, l].type = (ushort)ModContent.TileType<ParasiticNanitesSand>();
							WorldGen.SquareTileFrame(k, l, true);
							NetMessage.SendTileSquare(-1, k, l, 1);
						}
						else if (TileID.Sets.Conversion.Grass[type])
						{
							Main.tile[k, l].type = (ushort)ModContent.TileType<ParasiticNanitesGrass>();
							WorldGen.SquareTileFrame(k, l, true);
							NetMessage.SendTileSquare(-1, k, l, 1);
						}
					}
